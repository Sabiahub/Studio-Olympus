...
export default function TeamPage() {
  ...
  const handleDelete = async (id: string) => {
    ...
    if (error || !data || data.length === 0) {
      alert(`Não foi possível excluir: ${error?.message || 'Permissão negada ou artista não encontrado.'}`);
      return;
    }
    fetchArtists();
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingId) {
        ...
        if (error || !data || data.length === 0) {
          throw new Error(error?.message || 'Permissão negada ou artista não encontrado.');
        }
      } else {
